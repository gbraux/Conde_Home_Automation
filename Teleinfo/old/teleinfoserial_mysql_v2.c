...
#define MYSQL_PWD "raspberry"
 
// Fichier local au Wrt4gl/PC + fichier trame pour debug.
#define DATACSV "/home/pi/teleinfosql.csv"
...
// Déclaration pour le port série LLAP.
int             fdserial_llap ;
struct termios  termiosteleinfo_llap ;
...
char etiquettes[NB_VALEURS][17] = {"ADCO", "OPTARIF", "ISOUSC", "HCHP", "HCHC", "PTEC", "IINST1", "IINST2", "IINST3", "IMAX1", "IMAX2", "IMAX3", "PMAX", "PAPP", "HHPHC", "MOTDETAT", "PPOT", "ADIR1", "ADIR2" ,"ADIR3","TEMP","VDD"} ;

// Fin Constantes/variables à changées suivant abonnement.
 
char 	valeurs[NB_VALEURS][19] ;
char 	checksum[255] ;
int 	res ;
int	no_essais = 1 ;
int	nb_essais = 3 ;
...
//termiosteleinfo.c_cflag &= ~PARENB; // no parentybyte
//termiosteleinfo.c_cflag &= ~CSTOPB; // 1 stop bit
//termiosteleinfo.c_cflag &= ~CSIZE; // Mask the character size bits
//termiosteleinfo.c_cflag |= CS8; // 8 data bits
  ...
  termiosteleinfo.c_cc[VTIME] = 0 ;  				// Pas de time Out.
	termiosteleinfo.c_cc[VMIN]  = 1 ;   				// 1 car. attendu.
...
int initserie_llap(void)
// Mode Non-Canonical Input Processing, Attend 1 caractère ou time-out(avec VMIN et VTIME).
{
printf("LLAP Serial Loading \n");	
int device ;
 
        // Ouverture de la liaison serie (Nouvelle version de config.)
        if ( (device=open(SERIALPORT, O_RDWR | O_NOCTTY)) == -1 ) 
	{
		printf("Port Serie LLAP Erreur \n");                
		syslog(LOG_ERR, "Erreur ouverture du port serie LLAP %s !", SERIALPORT);
                exit(1) ;
        }

printf("Port Serie LLAP ouvert \n"); 

        tcgetattr(device,&termiosteleinfo_llap) ;				// Lecture des parametres courants.
 
	cfsetispeed(&termiosteleinfo_llap, BAUDRATELLAP) ;			// Configure le débit en entrée/sortie.
	cfsetospeed(&termiosteleinfo_llap, BAUDRATELLAP) ;
 
	termiosteleinfo_llap.c_cflag |= (CLOCAL | CREAD) ;			// Active réception et mode local.
 
	// Format série "7E1"
	termiosteleinfo_llap.c_cflag |= PARENB  ;				// Active 7 bits de donnees avec parite pair.
	//termiosteleinfo.c_cflag &= ~PARODD ;
	//termiosteleinfo.c_cflag &= ~CSTOPB ;
	//termiosteleinfo.c_cflag &= ~CSIZE ;
	//termiosteleinfo.c_cflag |= CS7 ;
//termiosteleinfo.c_cflag &= ~PARENB; // no parentybyte
termiosteleinfo_llap.c_cflag &= ~CSTOPB; // 1 stop bit
termiosteleinfo_llap.c_cflag &= ~CSIZE; // Mask the character size bits
termiosteleinfo_llap.c_cflag |= CS8; // 8 data bits
 
	termiosteleinfo_llap.c_iflag |= (INPCK | ISTRIP) ;			// Mode de control de parité.
 
	termiosteleinfo_llap.c_cflag &= ~CRTSCTS ;				// Désactive control de flux matériel.
 
	termiosteleinfo_llap.c_lflag &= ~(ICANON | ECHO | ECHOE | ISIG) ;	// Mode non-canonique (mode raw) sans echo.
 
	termiosteleinfo_llap.c_iflag &= ~(IXON | IXOFF | IXANY | ICRNL) ;	// Désactive control de flux logiciel, conversion 0xOD en 0x0A.
 
	termiosteleinfo_llap.c_oflag &= ~OPOST ;				// Pas de mode de sortie particulier (mode raw).
 
	termiosteleinfo_llap.c_cc[VTIME] = 80 ;  				// time-out à ~8s.
  //termiosteleinfo.c_cc[VTIME] = 0 ;  				// Pas de time Out.
	termiosteleinfo_llap.c_cc[VMIN]  = 1 ;   				// 1 car. attendu.
 
	tcflush(device, TCIFLUSH) ;					// Efface les données reçues mais non lues.
        tcsetattr(device,TCSANOW,&termiosteleinfo_llap) ;			// Sauvegarde des nouveaux parametres
printf("Serial LLAP Loaded \n");
	return device ;
}
...
int checksum_ok(char *etiquette, char *valeur, char checksum) 
{
    ...
    //printf("Checksum error : %s %s %s \n", etiquette, valeur, checksum);
		syslog(LOG_INFO, "Checksum lu:%02x   calculé:%02x / Etiquette:%s / Valeur lu:%s / Checksum lu:%c", checksum, sum, etiquette, valeur, checksum) ;
    //printf("Checksum lu:%02x   calculé:%02x / Etiquette:%s / Valeur lu:%s / Checksum lu:%c", checksum, sum, etiquette, valeur, checksum) ;
	#endif
	return 0 ;
}
...
int LitValEtiquettes()
{
  ...
  etiq_copy[0] = '\0';
	...
	for (id=0; id<NB_VALEURS; id++)
	{
		...
		{
			strcpy(etiq_copy,etiquettes[id]);
      sscanf(match, "%s %s %s", etiq_copy, valeurs[id], checksum) ;
    ...
    {
      syslog(LOG_ERR, "Etiquette %s manquante (essai %d) !\n", etiquettes[id], no_essais) ;
      return 0 ;
    }
	}
...
int DepasseCapacite()
{
	...
	{
		syslog(LOG_INFO, "Dépassement d'intensité: ADIR1='%s', ADIR2='%s', ADIR3='%s' !", valeurs[17], valeurs[18], valeurs[19]) ;
		return 1 ;
	}
	return 0 ;
}
...
/*------------------------------------------------------------------------------*/
/* Lit le timestamp de la derniere ligne mysql					*/
/*------------------------------------------------------------------------------*/
int readMysqlLastTimestamp()
{
	MYSQL mysql ;
	//char query[255] ;
	...
	if(mysql_query(&mysql, "SELECT timestamp FROM teleinfo ORDER BY timestamp DESC LIMIT 1"))
	{
		syslog(LOG_ERR, "Erreur SELECT TIMESTAMP :  %d: %s \n", mysql_errno(&mysql), mysql_error(&mysql));
		mysql_close(&mysql);
		return 0 ;
	}
...
{ 
    ...
    { 
        lastKnownTimestamp = atoi(row[i]);
        printf("Last timestamp : %u", atoi(row[i]));
...
openlog("teleinfoserial_mysql", LOG_PID, LOG_USER) ;
 ...
 do
 {
  ...
  // Reset Etiquettes
  //etiquettes[NB_VALEURS][17] = {"ADCO", "OPTARIF", "ISOUSC", "HCHP", "HCHC", "PTEC", "IINST", "IINST2", "IINST3", "IMAX", "IMAX2", "IMAX3", "PMAX", "PAPP", "HHPHC", "MOTDETAT", "PPOT", "ADIR", "ADIR2" ,"ADIR3","TEMP","VDD"} ;
  ...
  // Lit trame téléinfo.
	...
	{
		sprintf(datateleinfo,"'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s'", timestamp, sdate, sheure, valeurs[0], valeurs[1], valeurs[2], valeurs[3], valeurs[4], valeurs[5], valeurs[6], valeurs[7], valeurs[8], valeurs[9], valeurs[10], valeurs[11], valeurs[12], valeurs[13], valeurs[14], valeurs[15], valeurs[16], valeurs[17], valeurs[18], valeurs[19], valeurs[20],valeurs[21]) ;
		
    if (readMysqlLastTimestamp())
    {
    printf("Timestamp read OK\n");
      if ((lastKnownTimestamp + 240) < atoi(timestamp))
      {
      printf("Timestamp too okd - we write to SQL");
        if (! writemysqlteleinfo(datateleinfo) )
        {
        syslog(LOG_ERR, "Erreur MSQL - On stock en CSV");
        writecsvteleinfo(datateleinfo) ;		// Si écriture dans base MySql KO, écriture dans fichier csv.
        }
      }
    }
    else
