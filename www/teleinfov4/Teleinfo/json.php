...
function instantly () {
    ...
    if ($nbenreg > 0) {
        ...
        //$val["I"] = floatval(str_replace(",", ".", $row["IINST1"])); --> Modifs pour la temperature
		$val["I"] = round(floatval(str_replace(",", ".", $row["TEMP"])),1);
		$val["T"] = floatval(str_replace(",", ".", $row["TEMP"]));
        ...
        if ($nbenreg > 0) {
            ...
            //$max["I"] = max($val["I"], $row["IINST1"]); --> Modifs pour la temperature
			$max["I"] = max($val["I"], $row["TEMP"]);
			$max["T"] = max($val["T"], $row["TEMP"]);
        };
        mysql_free_result($result);
		...
		// MODIFICATIONS POUR LA TEMPERATURE !!
		//------------------------------------------------
        ...
        if (($graphConf["doubleGauge"])) {
            ...
            $series["I"] = "Temperature <br> Exterieure (C°)";
...
function daily () {
    ...
    if ($nbenreg > 0) {
        ...
        $seuils = array (
            'min' => round($plotlines_min),
            'max' => round($plotlines_max),
        );
